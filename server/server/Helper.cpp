...
std::string Helper::getUsername(SOCKET client_socket)
{

	string loginDetails = Helper::getStringPartFromSocket(client_socket, 2);
	string usernameLen = loginDetails;
	int userLenNum = (usernameLen[0] - 48) * 10 + (usernameLen[1] - 48);
	string userName = Helper::getStringPartFromSocket(client_socket, userLenNum);
	return userName;
}
